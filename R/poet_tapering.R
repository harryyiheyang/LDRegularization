...
#' @param tapering_vec Integer vector of candidate bandwidths for tapering.
...
#'
#' @return A covariance matrix (p×p) regularized by POET + tapering; symmetric
...
#' @export
poet_tapering=function(S,n,cutoff_method="D.ratio",k_min=5,k_max=min(15,round(nrow(S)/2)),tapering_vec=seq(2,20,2),eigenmin=1e-3){
p=ncol(S)
S[is.na(S)]=0;
diag(S)=1
S[abs(S)<0.0001]=0
