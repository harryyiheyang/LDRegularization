#' poet_thresholding: POET with entrywise thresholding on the idiosyncratic covariance
#'
...
#' @param thresholding_vec Numeric vector of candidate thresholding levels for
...
#'
#' @return A covariance matrix (p×p) regularized by POET + entrywise
...
#' @export
poet_thresholding=function(S,n,cutoff_method="D.ratio",k_min=5,k_max=min(15,round(nrow(S)/2)),thresholding_vec=seq(0.01, 0.20, by = 0.02),eigenmin=1e-3){
  p=ncol(S)
  S[is.na(S)]=0;
  diag(S)=1
  S[abs(S)<0.0001]=0
  ...
  E1=thresholding(E,lam_vec=thresholding_vec,eigenmin=eigenmin)
  hatS=P+E1
  return(cov2cor(hatS))
}
